...
class LLMManager:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "meta-llama/Meta-Llama-3.1-8B-Instruct", interaction_logger=None):
        """Initialize pure LLM manager."""
        import os
        # Use VLLM instead of OpenAI
...
STYLE: Clear, professional, friendly. Provide direct explanations for "what is" and "explain" questions. Adapt technical level to context."""
    ...
    def generate_with_functions_streaming(self, messages: List[Dict], system_prompt: str, 
        ...
        try:
            ...
            # Stream content word-by-word for smooth ChatGPT-like experience
            collected_content = ""
            word_buffer = ""
            ...
            for chunk in response:
                ...
                if chunk.choices[0].delta.content:
                    ...
                    word_buffer += chunk_content
                    
                    # Stream immediately for smooth flow (like ChatGPT)
                    # Send each chunk as it arrives for real-time streaming
                    yield {
                        'content': chunk_content,
                        'function_call': None,
                        'done': False
                    }
                    word_buffer = ""  # Clear buffer after sending
                    
                    # Add tiny delay to prevent buffering and create natural flow
                    # This helps ensure chunks are sent individually rather than buffered
                    time.sleep(0.002)  # 2ms delay for natural streaming effect
            ...
            # Send any remaining content (though should be empty with immediate streaming)
