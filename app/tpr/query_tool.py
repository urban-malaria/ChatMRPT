...
class QueryTPRData(DataAnalysisTool):
    ...
    def execute(self, session_id: str, **kwargs) -> ToolExecutionResult:
        ...
        from app.tpr.precompute import query_precomputed_tpr, is_tpr_precomputed
        from app.tpr.precompute_service import get_precompute_status
...
class SwitchTPRCombination(DataAnalysisTool):
    ...
    def execute(self, session_id: str, **kwargs) -> ToolExecutionResult:
        ...
        from app.tpr.cache import load_ward_cache, is_ward_cache_available
        from app.tpr.precompute import query_precomputed_tpr, is_tpr_precomputed
        ...
        # Save shapefile
        try:
            from app.tpr.analysis_tool import create_shapefile_package
        ...
        if self.generate_map:
            try:
                from app.tpr.analysis_tool import create_tpr_map
...
You can now run **risk analysis** or **ITN planning** using this combination."""
    ...
    def _build_final_dataset(self, merged_gdf: gpd.GeoDataFrame, env_data: pd.DataFrame, state_name: str) -> pd.DataFrame:
        ...
        from app.tpr.analysis_tool import get_geopolitical_zone
